# AWS Agentic Football Cup：基于 Rule-LLM 混合架构的智能体决策流水线优化
...
快速决策层的核心价值：在结果高度确定的场景跳过 LLM 推理，避免大模型决策的不确定性，同时降低耗时与算力消耗。
...
备注：LLM 本质上仍属于概率生成模型，即使通过 Prompt 或参数约束降低随机性，也无法替代基于环境状态约束的确定性规则。因此，在满足明确规则条件、动作选择具有高度确定性的场景中，直接使用规则执行更加稳定。
...
指令校验层作为统一执行入口，对快速决策层和 LLM 推理层产生的动作指令进行后置核验。
...
> 原有兜底机制仅拦截语法错误指令，本条 JSON 格式完全合规，会直接进入执行环节造成逻辑异常。新增语义校验层可以有效避免此类违反环境约束的指令进入执行阶段。
...
| 🧠 决策质量提升 | 规则处理确定性场景，LLM 处理战术不确定性 |
| 🛡 执行稳定性 | 校验层确保动作符合当前环境约束 |
