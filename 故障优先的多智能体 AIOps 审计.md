...
## 📌 项目背景
本项目是对 **AWS Bedrock AgentCore 多智能体 AIOps 工作坊** 的深度复现与架构审计。
...
通过对 `Investigation Workflow` 审计日志的对比分析，我识别出了生成式智能体在运维场景中的关键挑战：

### 1. 决策前置与先验偏差 (Pre-execution Bias)
* **审计实证**：在失败案例（2025-12-20）中，Brain Agent 在任务分发阶段即锁定了资源目标。
...
* **风险点**：AI 展现了较强的“归因偏好”，能拼凑出一套逻辑自洽的虚假根因。在无人值守场景下，这将导致错误的自动修复动作（如重启网关而非扩容数据库），进一步扩大故障范围。

### 3. 无反馈的推理循环 (Reasoning Loops)
* **审计实证**：当 Agent 在限定范围内找不到证据时，表现出重复查询同一数据源的倾向。
* **架构缺陷**：当前的编排缺乏 **“单调性检查”**。子 Agent 仅具备局部上下文，缺乏对“已失败尝试”的历史记忆，导致计算资源在重复迭代中损耗。
