...
* **审计实证**：在子Agent（Logs/Trace/Change）开始执行前，Brain Agent 已在任务分发阶段完成了资源绑定。
* **现象描述**：在失败案例（12-20）中，task-3 被静态绑定为 aiops-demo-sample-api（网关），而真正的故障源位于 Lambda。这种初始目标的偏移导致后续 Agent 产生了“视野盲区”，错失了底层限流的关键证据。
...
* **风险点**：在无人值守场景下，这将导致错误的自动修复动作（如扩容数据库），进一步扩大故障范围。

### 3. 无反馈的推理循环 (Reasoning Loops)
* **审计实证**：下游专家 Agent 严格遵守任务指令中的 Resource ID，当 Agent 在限定范围内找不到证据时，表现出重复查询同一数据源的倾向。
* **深度洞察**：缺乏 **“单调性检查”**。Agent 仅具备局部上下文，缺乏对“已失败尝试”的历史记忆，导致计算资源在重复迭代中损耗。并且即便日志中存在明确的异样（如 Throttling），受限于预设的 Resource ID 指令，Agent 无法跨越边界去发现真正的故障源，从而产生 **证据饥饿 (Evidence Starvation)**。
...
成功实验（00:30）：
Task 3: ChangeDetectionAgentQuery: CloudTrail for changes to **aiops-demo-put-item** in last 24 hours。
Found critical Lambda function configuration changes within 2 minutes of the 5XXError alarm. The aiops-demo-put-item Lambda function was updated by 'aiops-demo-error-injection' user 43 seconds before the alarm triggered.
Final RCA: Root cause identified: DynamoDB ProvisionedThroughputExceededException errors in aiops-demo-put-item Lambda function caused API Gateway 5XX errors. Lambda handled throttling exceptions but returned non-2xx responses that API Gateway interpreted as 5XX faults, despite Lambda reporting zero execution errors.
失败实验（01:37）：
Task 3: ChangeDetectionAgentQuery: Query CloudTrail for changes to **aiops-demo-sample-api** in last 24 hours。       <------Wrong Resource ID
No CloudTrail events were found for the API Gateway resource `aiops-demo-sample-api` during the 24-hour period. 
Final RCA: Root cause identified as DynamoDB ProvisionedThroughputExceededException throttling in the aiops-demo-post-item Lambda function. Logs confirm 753 throttling errors during sustained period originating from putItemHandler. Evidence shows both actual throttling and simulation messages indicating capacity limitations.
